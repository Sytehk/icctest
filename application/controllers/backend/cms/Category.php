...
class Category extends CI_Controller {
	...
	{  
		...
		$cat_news = $this->db->select('cat_id')->from('web_news')->where('cat_id', $cat_id)->get()->num_rows();
		$cat_cont = $this->db->select('cat_id')->from('web_article')->where('cat_id', $cat_id)->get()->num_rows();

		if ($cat_news>0 || $cat_cont>0) {
			$this->session->set_flashdata('exception', "This Category Containt Exist");
