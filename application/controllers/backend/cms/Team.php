...
class Team extends CI_Controller {
	...
	{  
        ...
        * Pagination ends
        ******************************/
        $data['team_overview'] = $this->db->select("*")
		                            ->from('web_article')
		                            ->where('article_id', 46)
		                            ->limit(1)
		                            ->get()
	...
	{
		$this->form_validation->set_rules('overviewheaderen', display('overview_header_en'),'required');
		$this->form_validation->set_rules('overviewheaderfr', display('overview_header_fr'),'required');
		$this->form_validation->set_rules('subheaderen', display('sub_header_en'),'required');
		$this->form_validation->set_rules('subheaderfr', display('sub_header'),'required');
		$this->form_validation->set_rules('descriptionen', display('description_en'),'required');
		$this->form_validation->set_rules('descriptionfr', display('description_fr'),'required');

		if($this->form_validation->run())
		{
			$overviewheaderen 	= $this->input->post('overviewheaderen');
			$overviewheaderfr 	= $this->input->post('overviewheaderfr');
			$subheaderen 		= $this->input->post('subheaderen');
			$subheaderfr 		= $this->input->post('subheaderfr');
			...
			$this->db->where('article_id',46)->update("web_article",$data);

			$this->session->set_flashdata('message','Update Successfully!');
		}
		else{
			$this->session->set_flashdata('exception','Update Faild!');
		}
