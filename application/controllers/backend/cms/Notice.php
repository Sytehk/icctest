...
class Notice extends CI_Controller {
	...
	{  
        ...
        * Pagination Start
        ******************************/
        $config["base_url"] = base_url('backend/cms/notice/index');
	...
	public function form($article_id = null)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if (empty($article_id)) 
			{
				...
				redirect("backend/cms/notice/form");
			...
			else 
			{
				...
				redirect("backend/cms/notice/form/$article_id");
		...
		$data['content'] = $this->load->view("backend/article/notice", $data, true);
		$this->load->view("backend/layout/main_wrapper", $data);
