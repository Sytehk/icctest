...
class FAQ extends CI_Controller {
	...
	{  
		...
		$ids = $this->db->select("cat_id")->from('web_category')->where('parent_id', $cat_id)->get()->result();
        ...
        * Pagination Start
        ******************************/
        $config["base_url"] = base_url('backend/cms/faq/index');
        $config["total_rows"] = $this->db->select('*')->from('web_article')->where_in('cat_id', $cat_ids)->get()->num_rows();
        $config["per_page"] = 25;
	...
	public function form($article_id = null)
	{
		...
		$this->form_validation->set_rules('article1_en', display('article1_en'),'required');
		...
		if ($this->form_validation->run()) 
		{
			...
			else 
			{
				...
				redirect("backend/cms/faq/form/$article_id");
			...
			$data['child_cat'] = $this->db->select("*")->from('web_category')->where('parent_id', $parent_cat->cat_id)->get()->result();
		...
		$data['content'] = $this->load->view("backend/article/faq", $data, true);
		$this->load->view("backend/layout/main_wrapper", $data);
