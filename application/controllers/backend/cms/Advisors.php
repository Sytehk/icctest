...
class Advisors extends CI_Controller {
	...
	{  
        ...
        * Pagination Start
        ******************************/
        $config["base_url"] = base_url('backend/cms/advisors/index');
	...
	public function form($article_id = null)
	{
		...
		if ($this->form_validation->run()) 
		{
			...
			if (empty($article_id)) 
			{
				...
				redirect("backend/cms/advisors/form");
			...
			else 
			{
				...
				redirect("backend/cms/advisors/form/$article_id");
		...
		$data['content'] = $this->load->view("backend/article/advisors", $data, true);
		$this->load->view("backend/layout/main_wrapper", $data);
