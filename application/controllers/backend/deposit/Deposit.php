...
class Deposit extends CI_Controller {
	...
	{  
        ...
        $config["total_rows"] = $this->db->count_all('dbt_deposit');
        $config["per_page"] = 25;
	...
	public function confirm_deposit()
	{
        ...
        $this->db->where('id',$id)->where('user_id',$user_id)->update('dbt_deposit',$deposit_data);




        $data = $this->db->select('*')->from('dbt_deposit')->where('id',$id)->get()->row();
        ...
        $userdata = $this->db->select('*')->from('dbt_user')->where('user_id', $user_id)->get()->row();
        ...
        if($set->deposit!=NULL){
            ...
            $post = array(
                ...
                'subject'  => 'Deposit',
                'to'       => $userdata->email,
                'message'  => 'You successfully deposit the amount $'.$data->amount.'. Your new balance is $'.$new_balance,
            );
            ...
            if($send_email){
                    ...
                    'details'           => 'You successfully deposit The amount $'.$data->amount.'. Your new balance is $'.$new_balance,
