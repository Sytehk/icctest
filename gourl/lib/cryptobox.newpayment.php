...
function cryptobox_new_payment($paymentID = 0, $payment_details = array(), $box_status = "")
{
	...
	if (isset($payment_details['status']) && @$payment_details['status'] == 'payment_received' && $box_status == "cryptobox_newrecord") {
		...
		if (!$recordExists) {
			...
			$fees = 0;
			$feesExists = run_sql("SELECT * FROM `dbt_fees` WHERE level = 'DEPOSIT'");
			if ($feesExists) {
			...
			run_sql("INSERT INTO `dbt_deposit` VALUES('','".$payment_details["user"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'bitcoin',".floatval($fees).",'".$paymentID."','".gmdate("Y-m-d H:i:s")."','',".intval(1).",'".@$_SERVER['REMOTE_ADDR']."','".$payment_details["coinname"]."')");


			// $processed = run_sql("select processed as nme FROM `crypto_payments` WHERE orderID = ".$orderID." LIMIT 1");


			$UserRecordExists = run_sql("SELECT * FROM `dbt_balance` WHERE user_id = '".$payment_details["user"]."'");

			if (!$UserRecordExists) {
				run_sql("INSERT INTO `dbt_balance` VALUES('','".$payment_details["user"]."',".(floatval($payment_details["amount"])-floatval($fees)).",'".gmdate("Y-m-d H:i:s")."')");

				$UserNewRecordExists = run_sql("SELECT * FROM `dbt_balance` WHERE user_id = '".$payment_details["user"]."'");

				if ($UserNewRecordExists) {
					run_sql("INSERT INTO `dbt_balance_log` VALUES('','".@$UserNewRecordExists->id."','".$payment_details["user"]."','DEPOSIT', ".(floatval($payment_details["amount"])-floatval($fees)).",'".floatval($fees)."', '".@$_SERVER['REMOTE_ADDR']."', '".gmdate("Y-m-d H:i:s")."')");
				}

			}else{

				$sql = "UPDATE `dbt_balance` SET `balance` = (`balance`+".(floatval($payment_details["amount"])-floatval($fees)).") WHERE user_id = '".$payment_details["user"]."' LIMIT 1";
				run_sql($sql);

				run_sql("INSERT INTO `dbt_balance_log` VALUES('','".@$UserRecordExists->id."','".$payment_details["user"]."','DEPOSIT', ".(floatval($payment_details["amount"])-floatval($fees)).",'".floatval($fees)."', '".@$_SERVER['REMOTE_ADDR']."', '".gmdate("Y-m-d H:i:s")."')");
			}
		}
